<script lang="ts">
	...
	import { Pencil, Trash2, Calculator, EllipsisVertical, Image } from 'lucide-svelte';
	import {
		DropdownMenu,
		DropdownMenuContent,
		DropdownMenuItem,
		...
		<div class="grid gap-4 py-4">
			...
			<div class="mt-4 rounded-lg bg-muted/50 p-4">
				...
				<div class="space-y-1 text-sm">
					...
					<p><strong>Carbs:</strong> {calculatedMacros.carbs} g</p>
				</div>
			</div>
		</div>
	</Dialog.Content>
...
<Dialog.Content class="sm:max-w-[425px]">
	...
	<div class="grid gap-4 py-4">
		...
		<div class="grid grid-cols-4 items-center gap-4">
			<Label for="carbs-edit-{product.id}" class="text-right">Carbs</Label>
			<Input
...
<Dialog.Root bind:open={isImageDialogOpen}>
	<Dialog.Content class="max-w-2xl">
		<Dialog.Header>
			...
			<Dialog.Description>
				Sube un archivo o pega una URL para extraer una como imagen del producto "{product.name}".
			</Dialog.Description>
		</Dialog.Header>
		<form
		method="POST"
