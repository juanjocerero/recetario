...
import type { CustomIngredient } from '@prisma/client';
...
&search_simple=1&action=process&json=1&page_size=5`;
  ...
  try {
    ...
    const newProduct: Prisma.ProductCreateInput = {
      ...
      fullPayload: productData as any,   // Prisma.JsonValue
    };
  ...
  for (const name of BASE_INGREDIENTS) {
    const ingredient = await prisma.customIngredient.create({
      data: {
        ...
        calories: parseFloat(faker.number.float({ min: 50, max: 500, fractionDigits: 1 }).toFixed(1)),
        protein: parseFloat(faker.number.float({ min: 0, max: 30, fractionDigits: 1 }).toFixed(1)),
        fat: parseFloat(faker.number.float({ min: 0, max: 50, fractionDigits: 1 }).toFixed(1)),
        carbs: parseFloat(faker.number.float({ min: 0, max: 100, fractionDigits: 1 }).toFixed(1))
      }
    });
